@extends('layouts.app')
@section('title', __('Delivery Mans'))

@section('content')
    <div class="d-flex align-items-center justify-content-center mb-3">
        <div class="flex-grow-1">
            <x-page-title>@lang('Delivery Mans')</x-page-title>
        </div>
    ...
    <x-card>
        <x-table id="delivery-table-">
            <x-thead>
                <tr>
                    <x-th>@lang('Name')</x-th>
                    <x-th>@lang('Phone')</x-th>
...
@push('script')
    <script>
        $(document).ready(function() {
            let dataTable = $('#delivery-table-').DataTable({
                processing: true,
                serverSide: true,
                language: {
                ...
                ajax: {
                    url: "{{ route('api.deliveries.index') }}",
                    dataSrc: 'data'
                },
                columns: [{
                        data: "name",
                    ...
                    },
                    {
                        ...
                        data: function(data, type, dataToSet) {
                            var editUrl = "{{ route('deliveries.edit', ':id') }}";
                            var deleteUrl = "{{ route('deliveries.destroy', ':id') }}";
