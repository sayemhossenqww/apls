...
class PurchaseController extends Controller
{
    ...
    {
        $purchases = Purchase::search(trim($request->search_query))->with([
        ...
        for ($i = 0; $i < count($purchases); $i++) {
            $purchases[$i]->average_cost = PurchaseDetail::where('purchase_id', $purchases[$i]->id)->avg('cost');
            $purchases[$i]->last_cost = PurchaseDetail::where('purchase_id', $purchases[$i]->id)->orderBy('id', 'desc')->first()->cost;
        }
    ...
    public function store(Request $request)
    {
        $request->validate([
            ...
            'shipment_name' => ['required', 'string', 'max:150'],
        ...
        $purchase->package_country = $request->package_country;
        $purchase->shipment_name = $request->shipment_name;
