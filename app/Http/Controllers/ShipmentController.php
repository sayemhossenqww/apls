   ...
   /**
    ...
    public function edit(Shipments $shipments): View
    {
        ...
        if(!empty($shipmentData))
        {
            ...
            if(!empty($shipmentData['supplier_name']) && is_array($shipmentData['supplier_name']))
            {
              $supplierName  = $shipmentData['supplier_name'];
            }else{
                $supplierName = json_decode($shipmentData['supplier_name']);
            }
           
            if(!empty($shipmentData['bar_code']) && is_array($shipmentData['bar_code']))
            {
                $barCode = $shipmentData['bar_code'];
            }else{
                $barCode = json_decode($shipmentData['bar_code']);
            }

            if(!empty($shipmentData['weight']) && is_array($shipmentData['weight']))
            {
              $weight = $shipmentData['weight'];
            }else{
                $weight = json_decode($shipmentData['weight']);
            }

            
            if(!empty($shipmentData['price']) && is_array($shipmentData['weight']))
            {
              $price = $shipmentData['price'];
            }else{
                $price = json_decode($shipmentData['price']);
            }
            //$shipmentData = SubShipments::where('shipment_id','=',$shipments['id'])->first();
